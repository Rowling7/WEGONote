...
set @settleTimeStart ='2024-03-01';
...
convert(decimal(18,2),sum(profitloss)/count(0)) 平均盈亏,
convert(decimal(18,2),sum(act_ipt_days )/convert(decimal(18,2), count(distinct jzlsh)) ) 平均住院天数,
convert(decimal(18,2),sum(totalcost)) 总费用,
convert(decimal(18,2),sum(totalcost)/count(0)) 平均费用,
convert(decimal(18,2),sum(a.west_drug_fee)) 西药费,
convert(decimal(18,2),sum(a.west_drug_fee)/count(0)) 平均西药费,
convert(decimal(18,2),sum(a.west_drug_fee)/sum(totalcost)*100) 西药费占比,
convert(decimal(18,2),sum(a.chinese_drug_fee)) 中成药费,
convert(decimal(18,2),sum(a.chinese_drug_fee)/count(0)) 平均中成药费,
convert(decimal(18,2),sum(a.chinese_drug_fee)/sum(totalcost)*100) 中成药费占比,
convert(decimal(18,2),sum(a.chinese_slice_drug_fee)) 中药饮片费,
convert(decimal(18,2),sum(a.chinese_slice_drug_fee)/count(0)) 平均中药饮片费,
convert(decimal(18,2),sum(a.chinese_slice_drug_fee)/sum(totalcost)*100) 中药饮片费占比,
convert(decimal(18,2),sum(a.drug_fee)) 药费,
convert(decimal(18,2),sum(a.drug_fee)/count(0)) 平均药费,
convert(decimal(18,2),sum(a.drug_fee)/sum(totalcost)*100) 药费占比,
convert(decimal(18,2),sum(a.consumable_fee)) 材料费,
convert(decimal(18,2),sum(a.consumable_fee)/count(0)) 平均材料费,
convert(decimal(18,2),sum(a.consumable_fee)/sum(totalcost)*100) 材料费占比,
convert(decimal(18,2),sum(a.check_fee)) 检查化验费,
convert(decimal(18,2),sum(a.check_fee)/count(0)) 平均检查化验费,
convert(decimal(18,2),sum(a.check_fee)/sum(totalcost)*100) 检查化验费占比,
convert(decimal(18,2),sum(a.medical_fee)) 医务性收入,
convert(decimal(18,2),sum(a.medical_fee)/count(0)) 平均医务性收入,
convert(decimal(18,2),sum(a.medical_fee)/sum(totalcost)*100) 医务性收入占比
from t_setlinfo a
